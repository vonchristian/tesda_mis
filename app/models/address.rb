class Address < ApplicationRecord
  ...
  def self.parse_address(address, client)
    if address.present? && address.split(",").count == 5
      Address.find_or_create_by(addressable: client, street: address.split(",").first(2).join.strip, barangay: Addresses::Barangay.find_or_create_by(name: address.split(",").third.strip), municipality_or_city: Addresses::MunicipalityOrCity.find_or_create_by(name: address.split(",").fourth.strip), province: Addresses::Province.find_or_create_by(name: address.split(",").last.strip))
    elsif address.present? && address.split(",").count == 4
      Address.find_or_create_by(addressable: client, street: address.split(",").first.strip, barangay: Addresses::Barangay.find_or_create_by(name: address.split(",").second.strip), municipality_or_city: Addresses::MunicipalityOrCity.find_or_create_by(name: address.split(",").third.strip), province: Addresses::Province.find_or_create_by(name: address.split(",").last.strip))
    elsif address.present? && address.split(",").count == 3
      Address.find_or_create_by(addressable: client, barangay: Addresses::Barangay.find_or_create_by(name: address.split(",").first.strip), municipality_or_city: Addresses::MunicipalityOrCity.find_or_create_by(name: address.split(",").second.strip), province: Addresses::Province.find_or_create_by(name: address.split(",").last.strip))
